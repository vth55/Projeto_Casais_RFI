...
**Objetivo Principal:** Rastrear utilização de equipamentos, cálculo de custos financeiros (€/h) e emissões de CO₂, e prever necessidades de manutenção baseado em **horas reais de uso**.
...
- 1º scan de um **DIFERENTE** cartão (com sessão aberta) → **SWITCH** (fecha a anterior e abre a nova automaticamente)
...
- **Automatic Switch:** Um novo operador pode interromper uma sessão ativa; o sistema fecha a anterior (`closeMethod: SWITCH`) e abre a nova sequencialmente.
- **Cálculo de Custos:** Cada sessão é vinculada ao tarifário em vigor na máquina no momento do scan (ver secção 10).
- Cartões não registados são bloqueados (403) e registados em `scan_buffer`.

---

### 3. **Lógica de Tarifários (Gestão de Custos)**
O sistema utiliza tarifários versionados para garantir auditoria histórica.
- **Só Máquina**: Combustível, manutenção e depreciação.
- **Máquina + Operador**: Inclui o custo proporcional de mão-de-obra.
- **Versionamento**: O Firestore guarda o `currentTariff` e um `tariffHistory[]`. Sessões fechadas guardam um *snapshot* do tarifário usado para evitar alterações retroativas indevidas.
...
**Eficiência por período (Taxa de Utilização):**
- **Fórmula:** `(Horas Trabalhadas (CLOSED) / (Nº Máquinas × Horas Úteis no Período)) × 100`
- **Capacidade Base:** 8h/dia, apenas dias úteis (Segunda a Sexta).
- **Cálculo de Dias:** Dinâmico via `getPeriodCapacityHours()` no store.
- **Hoje:** 8h disponíveis.
- **Semana:** Max 40h (5 dias × 8h).
- **Mês:** Baseado nos dias úteis decorridos até à data atual.
...
- **Roteamento Inteligente:** O ecrã "Dashboard" adapta-se à função do utilizador.
  - Gestores vêem rácios financeiros globais.
  - Técnicos de Manutenção vêem a saúde da frota num semáforo 🟢🟡🔴.
...
│
└── DOCS_ARCHITECTURE.md                 (Este ficheiro!)
```
