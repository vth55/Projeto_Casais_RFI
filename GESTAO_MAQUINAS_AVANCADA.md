...
- Cada máquina pertence a um **Tipo de Equipamento**
- **Gestão Flexível:** Sistema permite criar, editar e eliminar tipos conforme necessário
- **Tipos Iniciais:** Apenas alguns exemplos (ex: Escavadoras, Gruas)
- **Expansível:** Casais pode adicionar mais tipos depois conforme necessário
...
### **5. Gestão em Massa** ⚡
- Selecionar várias máquinas
- Aplicar valores uma vez (custos, emissões, etc.)
- Mecanismo de segurança (confirmação)

### **6. Padrões de Nomenclatura** 🏷️
- Máquinas da mesma categoria podem ter padrão idêntico
- Facilita identificação e organização
...
Máquinas da mesma categoria podem ter padrão idêntico no nome.

### **Exemplos:**

#### **Padrão 1: Categoria + Número + Localização**
- **Categoria:** Escavadoras (código: ESC)
- **Padrão:** `{CATEGORIA} {NÚMERO} - {OBRA}`
- **Exemplos:**
  - `ESC-01 - Obra Porto`
  - `ESC-02 - Obra Porto`
  - `ESC-01 - Obra Lisboa`

#### **Padrão 2: Categoria + Número Sequencial Global**
- **Padrão:** `{CATEGORIA}-{NÚMERO}`
- **Exemplos:**
  - `ESC-01`
  - `ESC-02`
  - `GRU-01` (Grua)

#### **Padrão 3: Nome Personalizado + Categoria**
- **Padrão:** `{NOME} - {CATEGORIA}`
- **Exemplos:**
  - `Escavadora Pesada - Escavadoras`
  - `Grua Torre - Gruas`

### **Sistema de Geração Automática:**

```javascript
// Função: generateMachineName(category, work, index)
function generateMachineName(category, work, index) {
  const pattern = category.namingPattern || "{CODE}-{NUM} - {WORK}";
  
  return pattern
    .replace("{CODE}", category.code)
    .replace("{NUM}", String(index).padStart(2, '0'))
    .replace("{WORK}", work.name)
    .replace("{CATEGORY}", category.name);
}

// Exemplo:
// generateMachineName(
//   { code: "ESC", name: "Escavadoras" },
//   { name: "Obra Porto" },
//   1
// )
// Resultado: "ESC-01 - Obra Porto"
```
...
// Firestore: machine_categories/{categoryId}
{
  ...
  name: "Escavadoras",
  code: "ESC",
  namingPattern: "{CODE}-{NUM} - {WORK}",  // Padrão opcional
  autoNumbering: true,  // Gerar número automaticamente?
  nextNumber: 3  // Próximo número disponível
}
```
